...
if($_GET['do'] == 'list') {
	if (is_dir($file)) {
		$directory = $file;
		$result = array();
		$files = array_diff(scandir($directory), array('.','..','profile'));
	    foreach($files as $entry) if($entry !== basename(__FILE__)) {
	        ...
	        $result[] = array(
	        	...
	        	'size' => $stat['size'],
	        	'name' => basename($i),
			'fileExt' => pathinfo($i, PATHINFO_EXTENSION),
	        	'path' => preg_replace('@^\./@', '', $i),
	        	'is_dir' => is_dir($i),
...
$(function(){
	...
	function renderFileRow(data) {
		...
		else if (data.fileExt == 'html')
		{
			dLink = "../textEditor/index.php?path=" + data.path;
		}
		else
		{
			dLink = './' + data.path;
		...
		var $html = $('<tr />').addClass(classType)
